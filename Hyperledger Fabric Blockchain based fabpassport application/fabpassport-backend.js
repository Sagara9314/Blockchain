...
// This method checks if the given passport exists and executes callback depending on success / failure
async function passportExists(passportID, success, failure){
    channel.queryByChaincode({
            chaincodeId: 'fabpassport',
            fcn: 'queryPassport',
            args: [passportID]
                    ...
                    () => {invoke(
                        {
                            chaincodeId: 'fabpassport',
                            fcn: 'createPassport',
                            args: [req.data.ID, req.data.fname, req.data.gender, req.data.dob, req.data.status, req.data.place],
                            chainId: 'mychannel',
                        }
                    , socket)});
                    break;
